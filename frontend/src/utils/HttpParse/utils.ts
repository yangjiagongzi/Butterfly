...
export const parseOrigin = (url: string, hostInHeader?: string) => {
  let host: URL | undefined = undefined;
  try {
    if (hostInHeader) {
      if (
        hostInHeader.startsWith("http://") ||
        hostInHeader.startsWith("https://")
      ) {
        host = new URL(hostInHeader);
      } else {
        host = new URL("https://" + hostInHeader);
      }
    }
  } catch (err) {
    console.log(err);
