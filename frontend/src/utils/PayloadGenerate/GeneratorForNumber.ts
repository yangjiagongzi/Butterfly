...
export default class NumberGenerator implements BaseGenerator {
  ...
  private formatNumber = (item: number, options: NumberGenerateOption) => {
    const value = Number(
      item.toFixed(options.isHex ? undefined : options.maxFractionDigits)
    );
    const [integer = "0", fraction = ""] = `${value}`.split(".");
    let integerFormat = Number(integer).toString(options.isHex ? 16 : 10);
    if (integerFormat.length > options.maxIntegerDigits) {
      integerFormat = integerFormat.slice(
        integerFormat.length - options.maxIntegerDigits
      );
    }
    ...
    if (fractionFormat.length < options.minFractionDigits) {
      fractionFormat = `${fractionFormat}0000000000`.slice(
        0,
        options.minFractionDigits
      );
    }
    const fullFormat = `${integerFormat}${
      fractionFormat.length ? `.${fractionFormat}` : ""
    }`;
